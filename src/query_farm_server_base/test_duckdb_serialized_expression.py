...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "struct_pack(hello := struct_extract(array_extract(\"Buckets\", 1), 'Name'), d := 19754, z := [1, 2, 3], c := ['f', null, 'b']) = {'hello':'foo','d':3652,'z':[4, 5, 6],'c':['f', null, 'b']} AND \"aws_region\" = 'us-east-1'",
            bucket_type_info,
        ),
        (
...
def test_duckdb_json_to_sql(json_source: str, expected_sql: str, expected_bound_types: dict[str, Any]) -> None:
    data = json.loads(json_source)
    result = duckdb_serialized_expression.convert_to_sql(
